...
class APIService:
    ...
    def process_audio(self, audio_path: str, meeting_date: Optional[str] = None) -> Dict:
        try:
            ...
            # 회의 요약
            summary = self.langchain_util.summarize_meeting(transcript)
            print(summary)
            # 일정 추출
            schedule = self.langchain_util.extract_schedule(transcript, meeting_date)
            print(schedule)
            # schedule 날짜 포메팅
            schedule = self.date_util.process_schedule_dates(schedule)

            # 할 일 추출
            todos = self.langchain_util.extract_todos(transcript, meeting_date)
            print(todos)
            # todos 날짜 포메팅
